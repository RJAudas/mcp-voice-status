...
- [ ] T006 Implement config loading in .github/hooks/scripts/voice-status-common.ps1: function Get-VoiceStatusConfig that reads voice-status-config.json (from script directory), merges with built-in hardcoded defaults for any missing fields, and parses env var overrides (VOICE_STATUS_RATE_LIMIT_MS, VOICE_STATUS_DEDUP_WINDOW_MS, VOICE_STATUS_TIMEOUT_MS, VOICE_STATUS_VOLUME, VOICE_STATUS_RATE) as string-to-int conversions with fallback to JSON value on parse failure. Basic range clamping (e.g., rateLimitMs min 1000) included here. T038 adds strict edge-case hardening.
...
- [ ] T008 Implement fire-and-forget TTS in .github/hooks/scripts/voice-status-common.ps1: function Invoke-Speech that takes sanitized text and config, builds a PowerShell script block using Add-Type System.Speech.Synthesis with configurable Rate and Volume, and launches it via Start-Job. The script block MUST use SpeakAsync() followed by WaitUntilDone(ttsTimeoutMs) internally — this ensures the background job self-terminates after the configured timeout (default 30s) even if speech hangs, satisfying Constitution VI. Invoke-Speech returns immediately after Start-Job without waiting for the job to complete (fire-and-forget, FR-016).
...
- [ ] T014 Write Pester unit tests for message summarization in tests/voice-status-common.Tests.ps1: test Get-ToolSummary for edit (extracts filename), create (extracts filename), bash with test output ("All 15 tests passed" → "15 tests passed"), bash with failure output ("2 tests failed"), bash with lint output ("0 errors, 2 warnings" → "Lint: 2 warnings"), bash with build output ("Build succeeded"/"Build failed"), bash with generic output ("Command completed"), and unknown tool names (returns $null or empty to trigger silent skip)
...
- [ ] T030 [US3] Wire rate limiting and dedup into the three non-error hooks: update on-session-start.ps1, on-session-end.ps1, on-post-tool-use.ps1 to call Test-RateLimited and Test-IsDuplicate before Invoke-Speech. If rate limited or duplicate, exit 0 silently. Call Update-SpeechState after successful speech. Note: on-error.ps1 bypasses rate limiting by design (handled in T033); on-prompt-submitted.ps1 wires its own rate limiting in T036.
...
- [ ] T038 [US6] Harden env var edge cases in Get-VoiceStatusConfig (beyond T006): add tests and handling for empty string env vars (treat as unset), negative values (reject, use default), out-of-range values (clamp: rateLimitMs 1000–60000, dedupWindowMs 1000–300000, volume 0–100, rate -10–10, ttsTimeoutMs 5000–120000), and conflicting JSON+env combinations. This task hardens the logic built in T006 — do not rewrite it.
...
- [ ] T042 Run full Pester test suite via Invoke-Pester -Path tests/ and fix any failures. Also verify SC-001 timing: run Measure-Command { '{"timestamp":"2026-01-01","cwd":".","source":"new","initialPrompt":"test"}' | .github/hooks/scripts/on-session-start.ps1 } and assert TotalMilliseconds < 2000. Repeat for on-post-tool-use.ps1 and on-error.ps1.
